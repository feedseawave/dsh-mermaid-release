...
var ClientModuleRegistry = class extends Service {
	...
	*/
	...
	serveBundle = async (req, res) => {
		...
		].find(({ suffix }) => pathname.startsWith(prefix) && pathname.endsWith(suffix));
		const clientPath = artifact === void 0 ? void 0 : this.clientPath(pathname.slice(9, -artifact.suffix.length));
		const path = clientPath === void 0 || artifact === void 0 ? void 0 : artifact.file === "" ? clientPath : artifact.file.startsWith(".") ? `${clientPath}${artifact.file}` : join(dirname(clientPath), artifact.file);
		if (path === void 0 || artifact === void 0) {
			res.writeHead(404);
			res.end();
			return;
		}
